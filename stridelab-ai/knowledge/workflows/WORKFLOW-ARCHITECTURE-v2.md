...
- **Automated documentation verification** (`stridelab-ai/scripts/validate-workflow-architecture.mjs`, added 2026-09-07): re-checks the 86 workflow IDs (unique, contiguous, per-category counts), count reconciliation across the canonical entry point / this spec / the registry, the seven required per-workflow contract fields, the 14-seam cross-context contract register (8 fields each), internal repo-path resolution in the canonical documents, absence of a live pointer to a superseded v1 filename, canonical status-string consistency, the 16 §5 invariants, the DD-DEPARTED-CONTENT back-references, and that the registry / gate / routing machine-readable files parse. Result: **PASS** on the finalization commit. Runs in CI via `.github/workflows/workflow-architecture-validation.yml` and under `npm run ai:validate`.
...
- Commit `393f60ce6ce59be23afd6c5290a7c39df7eb9ba7` (the v2 correction pass) is present on **both** `main` and the phase branch. It was committed directly; **there is no pull request and no G7 gate behind it.** Its presence on `main` is a version-control fact only and does **not** constitute G7 approval — the artifact stays `AWAITING HUMAN APPROVAL` until a human approves it.
- Default branch is `main` (correct). The stale `phase-1/workflow-architecture-v1` branch holds no unique commits and its name contradicts the v2 version; it is removed once the finalization PR is open (fully recoverable — same SHA is on `main`).
- Automated documentation verification was added: `stridelab-ai/scripts/validate-workflow-architecture.mjs` (workflow IDs / counts / duplicate IDs / required contract fields / 14-seam contract register / internal path resolution / stale-version references / canonical status consistency / 16 invariants / DD-DEPARTED-CONTENT back-references / registry+gate+routing parse), wired into `npm run ai:validate` and run on the PR by `.github/workflows/workflow-architecture-validation.yml`.
