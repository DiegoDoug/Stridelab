...
- `main` **branch protection** now requires a pull request and the `validate` status check before any merge.
- Automated documentation verification was added: `stridelab-ai/scripts/validate-workflow-architecture.mjs` — checks workflow IDs / counts / duplicate IDs / required contract fields / 14-seam contract register / internal path resolution / stale-version references / canonical status consistency / 16 invariants / DD-DEPARTED-CONTENT back-references, and **parses every machine-readable file (registry, gates, routing, approvals) with a real YAML parser (`yaml` devDependency), asserting the parsed structure and failing closed on malformed input** (negative-tested). It is invoked through `npm run ai:validate` (the real developer entry point), which `.github/workflows/workflow-architecture-validation.yml` runs on the PR after `npm ci`.
- G4 (Implementation Integrity) was executed by Department 05 against the finalization tooling diff — `PASS` (see the Gate summary).
- MG-07 was given an explicit **Authorization assumptions** block (owner edits only their own preferences; safety class not member-configurable by anyone; preference state never an authorization input) — the one workflow of 86 that previously stated this only inline.
- `docs/product/domain-model.md`: the blocking-stage description was corrected — it blocks **G4 sign-off per bounded context**, does **not** block G7 of this artifact, and does **not** block the 14 seam identifications completed at G2 (only their shape design at G4). The earlier "before API contracts … but does not block contract-shape design" self-contradiction is removed.
...
| G4 — Implementation Integrity | 05-quality-infrastructure-release | `PASS` (finalization tooling only) | v2 workflow content = no code/config/data change. The v2 finalization pass added a documentation validator, a read-only CI workflow, and `package.json`/`package-lock.json` (`yaml` devDependency); D05 executed G4 against that diff — validator implements its stated contract, parses all machine-readable files with a real YAML parser, fails closed on malformed input (negative-tested), passes locally and in CI on the head SHA, changes no boundary/decision/gate. |
