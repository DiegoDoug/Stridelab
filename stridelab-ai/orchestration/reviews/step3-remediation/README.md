...
| **RP-B4** | **BLOCKING** | quality-engineering | `quality-release-lead` | **CLOSED** | The extended validator passed against the working tree but **failed against a clean clone of the pushed commit**: every section-extraction regex anchors on `\n\n` / `\n\n---`, which does not match a CRLF checkout (Windows CI, or any clone with `core.autocrlf=true`). It reported §1.8 and §4 "not found", 0 destinations parsed, and ~55 spurious coverage errors for a correct artifact. Remediated at the source — `read()` normalises CRLF to LF, making every check line-ending agnostic (`c54735e`). Re-verified: full suite green on a fresh CRLF clone. |
...
**Zero open BLOCKING findings.** RP-B1…RP-B4 are closed and independently re-verified (`14-reverify.md`).
