# /verify-client-feature

**Department:** Client & Feature Engineering  
**Command group:** `verification`  
**Owning agent:** `client-verification-reviewer`  
**Command class:** `review`

## Intent

Execute the approved StrideLab workflow for **verify client feature** using the owning agent and required core skills.
...
`stridelab-ai/departments/02-client-feature-engineering/skills/core/<skill-name>/`
...
- confirm the task falls within `client-verification-reviewer` ownership;
...
- Client authorization checks are UX/defense-in-depth only; server-side enforcement remains authoritative.
- Preserve the distinction between coach prescription and athlete-performed workout records.
