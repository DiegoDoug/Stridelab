# /audit-coach-athlete-communications

**Department:** Security, Identity & Compliance  
**Command group:** `trust-safety`  
**Owning agent:** `youth-trust-safety-specialist`  
**Command class:** `review`

## Intent

Execute the approved StrideLab workflow for **audit coach athlete communications** using the owning agent and required core skills.
...
`stridelab-ai/departments/04-security-identity-compliance/skills/core/<skill-name>/`
...
- confirm the task falls within `youth-trust-safety-specialist` ownership;
...
1. Define review scope and authoritative acceptance criteria.
2. Inspect the actual artifact/code/runtime evidence; do not review from summaries alone when direct evidence exists.
3. Classify findings as blocking or non-blocking and cite evidence.
4. Return an explicit disposition such as PASS, PASS WITH CONDITIONS, or FAIL when the owning skill permits it.
...
- Treat athlete profile ownership, Event Group scope, private/tagged media visibility, and age 13+ safeguards as hard baselines.
- Legal/compliance outputs are research/requirements, not a substitute for qualified counsel.

## Output contract

A review/verification artifact with scope, evidence, blocking findings, non-blocking findings, and disposition.
