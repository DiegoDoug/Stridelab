...
**Upstream (Phase A):** `docs/product/feature-prioritization.md` (`APPROVED`; same joint G7 record and approved commit pin; Department 01 review PASS WITH CONDITIONS).

This artifact is the **approved governed MVP release scope** and ordered implementation sequence derived from the approved Phase A prioritization. No `OQ-*` / `CD-*` item is resolved, converted, or created by this approval; every conservative safe default remains in force. No product invariant is changed or contradicted. No navigation, schema, API, or UI is designed.
...
| Upstream (approved) | `feature-prioritization` (`APPROVED` under the same joint G7) — feature IDs `F-*`, `S-*`, `F-P-*`, `F-EX-*` are defined there |
...
| **G7 Product-scope Human Approval** | This scope + `feature-prioritization.md` + the full cross-department review (§13) with zero open BLOCKING findings + the decision package (§15). | **Explicit human approval recorded** — granted by Diego on 2026-09-10 for implementation-design only. |
| **G8 Production Verification** | Separate production-release G7 approved; release deployed. | Critical journeys (J-1…J-13) verified in production; telemetry healthy; no blocking regression; release record updated. |

### 11.2 Production-launch legal sign-offs (named conditions on G3 and the future production-release G7 — safe defaults are in force meanwhile)

`OQ-IT01-AGE-VERIFY` (age-verification strength) · `OQ-PS07-JURISDICTION` (duties on discovering an under-13 user) · `OQ-PS08-JURISDICTION` (data-subject-rights jurisdictions + SLAs) · `OQ-PS09-EXTERNAL` (external-reporting obligations + the legal-guidance process for any handoff). Each must be **obtained, or explicitly accepted as a conservative-default launch condition, in the separate future production-release G7 approval** — a youth product does not launch with these merely defaulted and unacknowledged (§13 finding CDR-B1). The Department 01 Step-2 product-scope G7 does not satisfy this requirement.
...
6. Every `§9.2` OPEN item that **gates an included capability** is either resolved **or** its conservative safe default is explicitly accepted for non-production implementation by this scope record and, where it affects launch, separately dispositioned at the future production-release G7 (the §14 register); no OPEN item was silently converted to an assumption.
7. The **production-launch legal sign-offs** (§11.2) are obtained or explicitly accepted as named launch conditions at the future production-release G7.
...
- **BLOCKING findings:**
  - **CDR-B1** *(BLOCKING; remediated)* — an earlier draft's "Definition of MVP completion" (§11.3) did not require the **production-launch legal sign-offs** (`OQ-IT01-AGE-VERIFY`, `OQ-PS07-JURISDICTION`, `OQ-PS08-JURISDICTION`, `OQ-PS09-EXTERNAL`) to be **obtained or explicitly accepted at the future production-release G7**. A youth product cannot be declared "MVP complete / ready for launch" with jurisdiction-specific age-verification and under-13 / data-subject / illegal-content-reporting duties merely defaulted and unacknowledged. **Evidence:** `WORKFLOW-ARCHITECTURE-v2.md` §9.2 rows for those IDs ("blocking stage: production launch legal sign-off" / "production moderation ops"); baseline §8 ("none is an approved legal interpretation"). **Remediation applied:** §11.2 added ("Production-launch legal sign-offs" — named conditions on G3 and the future production-release G7); §11.3 clause 7 added; the decision package §15 lists them as explicit future conditions. **Re-run:** cross-department re-review confirms the completion definition now cannot be met without addressing them → finding **closed**.
...
## 15. G7 decision outcome

Diego granted the human G7 product-scope approval on 2026-09-10 after drafting, remediation, verification, and the independent specialist review pass completed. **C-IND is closed** — all eight independent lenses (D01, D02, D03, D04, D05, D06, architecture, cross-department) are recorded in `stridelab-ai/orchestration/reviews/step2-dept01/`, the three that returned `FAIL` were remediated and **re-verified closed** (`09-reverify.md`), and zero BLOCKING findings remain open (§13.10, §13.11). This is a product-scope lock for implementation-design only. It is **not** a legal review, **not** a compliance sign-off, **not** launch authorization, and **not** production-release authorization (§11.4). The §13.10 conditions remain assigned to their later G2 / G4 / G5 / G6 stages under their safe defaults, and production launch still requires the §11.2 sign-offs plus a separate future G7 production-release decision.

| Field | Value |
|---|---|
| **Artifacts** | `docs/product/feature-prioritization.md` (Phase A) and `docs/product/mvp-release-scope.md` (Phase B — this file). |
| **Versions** | Both `APPROVED`, authored 2026-09-10; independent-review remediation pass 2026-09-10; approved version pinned to `9893c1d4dd0166fd0c55f1950e601e5f8737946c`. |
| **Approved repository state** | Commit `9893c1d4dd0166fd0c55f1950e601e5f8737946c` on branch `phase-1/dept01-step2-feature-prioritization-release-scope` / PR #9 (built on `main`@`5db47f1`, which contains the APPROVED consolidated domain model published at `760ca3c` / PR #6). Approval metadata is recorded in `stridelab-ai/orchestration/approvals/G7-mvp-feature-prioritization-release-scope.md` and both lifecycle records. |
...
- Governance: `stridelab-ai/orchestration/ORCHESTRATOR.md`, `stridelab-ai/orchestration/phase-gates/gates.yaml`, `stridelab-ai/orchestration/approvals/` (`approval-policy.md`, `approval-matrix.yaml`, `G7-mvp-feature-prioritization-release-scope.md`), `stridelab-ai/shared/contracts/review-contracts/REVIEW-CONTRACT.md`
