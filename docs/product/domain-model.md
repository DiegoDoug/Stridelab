...
| **Precise blocking stage** | Blocks **G4 (Implementation Integrity) sign-off for any bounded context** — no schema, API contract shape, or client-feature design may be *approved/merged* for a context until the consolidated domain model exists. It does **not** block G7 approval of Workflow Architecture v2, and it does **not** block the *seam identification* already completed at G2 (the 14 cross-context contract seams in `stridelab-ai/application-map/cross-context-contracts.md`); it is an input to turning those seams into concrete shapes at G4. |
